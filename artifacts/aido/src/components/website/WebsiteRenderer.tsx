...
import { Calendar, MapPin, Heart, Clock, Gift, HelpCircle, Image as ImageIcon, ChevronLeft, ChevronRight, X, ExternalLink, Navigation, CheckCircle2, Copy, Check } from "lucide-react";
...
function ShareButtons({ data }: { data: WebsiteRendererPayload }) {
  const [copied, setCopied] = useState(false);
  const url = typeof window !== "undefined" ? window.location.href : "";
  const couple = `${data.couple.partner1Name} & ${data.couple.partner2Name}`;
  const text = `Join us to celebrate the wedding of ${couple}!`;
  ...
  return (
    <div className="py-8 px-6 flex flex-col items-center gap-4" style={{ background: data.colorPalette.background }}>
      <p className="text-xs uppercase tracking-widest opacity-40" style={{ color: data.colorPalette.text }}>
        Share this website
      </p>
      <div className="flex flex-wrap items-center justify-center gap-3">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-medium transition-opacity hover:opacity-75"
          style={{ background: "#25D36612", border: "1.5px solid #25D366", color: "#25D366" }}
        ...
        <a
          ...
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-medium transition-opacity hover:opacity-75"
          style={{ background: "#1877F212", border: "1.5px solid #1877F2", color: "#1877F2" }}
        ...
        <button
          onClick={copy}
          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-medium transition-opacity hover:opacity-75"
          style={{ background: `${data.colorPalette.primary}12`, border: `1.5px solid ${data.colorPalette.primary}`, color: data.colorPalette.primary }}
        >
...
function Footer({ data, ctx }: { data: WebsiteRendererPayload; ctx: EditCtx }) {
  ...
  return (
    <>
      {!ctx.editable && <ShareButtons data={data} />}
