    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="wedding-party" className="mt-6">
          <Suspense fallback={<div className="space-y-3"><Skeleton className="h-10 w-64" /><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{[1,2,3,4].map(i => <Skeleton key={i} className="h-36 rounded-xl" />)}</div></div>}>
            <WeddingParty />
          </Suspense>
        </TabsContent>
      </Tabs>
