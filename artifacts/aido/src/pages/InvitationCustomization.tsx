...
export default function InvitationCustomizationPage({
  ...
  const digCustomColors = (() => {
    const bg = digitalInvitationBackground || "#1E1A2E";
    const c = bg.replace("#", "");
    const isLight = c.length === 6
      ? parseInt(c.slice(0, 2), 16) * 0.299
        + parseInt(c.slice(2, 4), 16) * 0.587
        + parseInt(c.slice(4, 6), 16) * 0.114 > 160
      : false;
    return {
      bg,
      accent: displayPalette.accent || "#D4A017",
      text: displayPalette.primary || (isLight ? "#1a1a1a" : "#ffffff"),
      muted: displayPalette.neutral || (isLight ? "rgba(0,0,0,0.58)" : "rgba(255,255,255,0.58)"),
