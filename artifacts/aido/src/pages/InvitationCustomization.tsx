...
export default function InvitationCustomizationPage({ profileId: propProfileId }: InvitationCustomizationProps = {}) {
  ...
  // Start true so the first auto-save effect run on mount is skipped.
  // Also set to true when we bulk-load state from the DB so we don't
  // immediately re-save what we just fetched.
  ...
  useEffect(() => {
    if (customization) {
      // Tell the auto-save effect to skip the next run so we don't immediately
      // re-write to the DB what we just fetched.
  ...
  // Reset every text-element override back to the brand-color defaults.
  // Clears position, font, color, and size overrides for all elements in both
  // previews so they naturally fall back to the current palette colours.
  const handleResetToBrandColors = () => {
    setTextOverrides({});
    // skipNextAutoSave is false here (user clicked a button), so the auto-save
    // effect will fire and persist the cleared state.
  };

  // Auto-save customizations when any setting changes.
  // • We skip the very first run on mount (skipNextAutoSave starts true).
  // • We also skip the run immediately after loading from the DB (the load
  //   effect sets skipNextAutoSave = true before calling the state setters).
  // • Photo uploads are kicked off directly from the selection handler; once
  //   the upload finishes, the URL flips from blob: to the server URL and this
  //   effect persists it. We skip while a blob: URL is in flight.
  useEffect(() => {
    if (!profileId) return;
        ...
        <div className="lg:col-span-2">
          <Card className="h-[90vh] flex flex-col">
            <div className="border-b p-4 space-y-3">
              {/* Reset button — always visible at the top of the preview panel */}
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  title="Clear all text overrides and restore brand colours"
                >
