...
export default function InvitationCustomizationPage({ profileId: propProfileId }: InvitationCustomizationProps = {}) {
  ...
  const [saveTheDateBackground, setSaveTheDateBackground] = useState<string | null>(null);
  ...
  const [digitalInvitationBackground, setDigitalInvitationBackground] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (customization) {
      ...
      // Per-design background
      setSaveTheDateBackground(customization.saveTheDateBackground ?? customization.backgroundColor ?? null);
      setDigitalInvitationBackground(customization.digitalInvitationBackground ?? customization.backgroundColor ?? null);
  ...
  // ── Reset to brand colors (active design only) ────────────────────────────
  const handleResetToBrandColors = () => {
    if (previewTab === "saveTheDate") setStdTextOverrides({});
    else setDigTextOverrides({});
  };
    ...
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4 max-h-[90vh] overflow-y-auto">
          ...
          {/* Font / layout / background — per-design, switches with the preview tab */}
          <DesignOptionsSection
            mode={previewTab}
            font={isSTD ? saveTheDateFont : digitalInvitationFont}
