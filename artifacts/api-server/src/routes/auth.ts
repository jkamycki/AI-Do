...
// requester did not own. Signup and sign-in are now done entirely via Clerk's
// frontend SDK, where email-code verification proves ownership before any
// session is issued.
//
...
const gone: import("express").RequestHandler = (_req, res) => {
  res.status(410).json({
    error:
      "This endpoint has been removed. Authentication now happens through Clerk-managed client flows.",
  });
};

router.post("/auth/signup", gone);
router.post("/auth/signin", gone);
