...
function isContractLookupQuestion(text: string): boolean {
  return /\b(contract|agreement|clause|clauses|key terms?|red flags?|cancellation|refund|liability|insurance|payment|deposit|balance|due)\b/i.test(text)
    && /\b(what|when|how much|tell me|show me|say|says|next|due|owe|owed|payment|pay|terms?|red flags?|risk|cancel|refund|liability)\b/i.test(text);
}
...
function buildContractToolReply(lastUserText: string, actions: ActionRecord[]): string | null {
  ...
  const summary = contractText(contract.summary);
  const risk = contractText(contract.overallRiskLevel, "unknown");
  const keyTerms = Array.isArray(contract.keyTerms) ? contract.keyTerms as Array<Record<string, unknown>> : [];
  ...
  const lines = [
    `Here is what **${label}** says based on the saved contract analysis:`,
    "",
    summary && summary !== "Not specified"
      ? summary
      : "I do not see a plain-language summary saved for this contract yet.",
    "",
    `**Risk level:** ${risk ?? "unknown"}`,
  ];

  if (paymentDetail && paymentDetail !== "Not specified") {
    lines.push("", `**Payment terms:** ${paymentDetail}`);
  }
  if (cancellation && cancellation !== "Not specified") {
    lines.push("", `**Cancellation/refund:** ${cancellation}`);
  }
  if (liability && liability !== "Not specified") {
    lines.push("", `**Liability/insurance:** ${liability}`);
  }
  if (keyTerms.length) {
    lines.push(
      "",
      "**Key terms:**",
      ...keyTerms.slice(0, 5).map((term) => `- **${contractText(term.label, "Term")}**: ${contractText(term.value, "Not specified")}`),
    );
  }
  if (redFlags.length) {
    lines.push(
      "",
      "**Red flags:**",
      ...redFlags.slice(0, 3).map((flag) => `- **${contractText(flag.severity, "risk")} - ${contractText(flag.title, "Issue")}**: ${contractText(flag.detail, "")}`),
    );
  }
  lines.push("", "I am generated by AI and not a lawyer, so review the original contract carefully before signing or making legal decisions.");
  return lines.join("\n");
}

async function buildDirectContractReply(req: Request, lastUserText: string): Promise<{ reply: string; action: ActionRecord } | null> {
  const result = await executeTool("get_contract", {}, req, { recentUserText: lastUserText.toLowerCase() });
  const action = { name: "get_contract", args: {}, result };
  if (!result.ok) {
    return {
      action,
      reply: result.error === "No uploaded contracts found."
        ? "I don't see any uploaded contracts yet. Upload one in Contract Analyzer first, then I can answer questions about payments, clauses, red flags, cancellation, and liability."
  ...
  try {
    ...
    if (name === "list_contracts") {
      ...
      const rows = await db
        ...
        .where(eq(vendorContracts.profileId, profile.id))
        .orderBy(desc(vendorContracts.createdAt))
        .limit(50);
...
router.post("/aria/chat", requireAuth, aiLimiter, async (req, res) => {
  try {
    ...
    if (isContractLookupQuestion(lastUserText)) {
      const contractReply = await buildDirectContractReply(req, lastUserText);
      if (contractReply) {
        send({ type: "action_start", name: "get_contract", args: {} });
        send({
          type: "action_result",
          name: "get_contract",
          ok: contractReply.action.result.ok,
          data: contractReply.action.result.ok ? contractReply.action.result.data : undefined,
          error: contractReply.action.result.ok ? undefined : contractReply.action.result.error,
        });
        send({ type: "content", content: contractReply.reply });
        send({ type: "done", actions: [{ name: "get_contract", ok: contractReply.action.result.ok, error: contractReply.action.result.ok ? undefined : contractReply.action.result.error }] });
        res.write("data: [DONE]\n\n");
        res.end();
        return;
      }
